...
function getName(id: string, participants: Participant[]): string {
 ...
 */
export function generate1v1Matches(participants: Participant[]): Match[] {
    const matches: Match[] = [];
 ...
 */
export function generateRotating2v2Matches(participants: Participant[]): Match[] {
    ...
    playersWithBye.forEach(id => {
        if (id !== "BYE") playerMatches[id] = 0;
    });

    const used = new Array(teams.length).fill(false);
    let madeMatch: boolean;
    ...
    do {
        ...
        for (let a = 0; a < teams.length; a++) {
            ...
            if (hasBye && (ta.p1 === "BYE" || ta.p2 === "BYE") && minUsageA !== Infinity) continue;

            let usage = 0;
            if (ta.p1 !== "BYE") usage += playerMatches[ta.p1]!;
            if (ta.p2 !== "BYE") usage += playerMatches[ta.p2]!;
        ...
        for (let b = 0; b < teams.length; b++) {
            if (used[b] || b === bestTeamA) continue;
            const tb = teams[b]!;

            // Ensure disjoint sets
            if (ta.p1 !== tb.p1 && ta.p1 !== tb.p2 && ta.p2 !== tb.p1 && ta.p2 !== tb.p2) {
                let usage = 0;
                if (tb.p1 !== "BYE") usage += playerMatches[tb.p1]!;
                if (tb.p2 !== "BYE") usage += playerMatches[tb.p2]!;
        ...
        if (bestTeamB !== -1) {
            ...
            if (teamA.p1 !== "BYE") playerMatches[teamA.p1]++;
            if (teamA.p2 !== "BYE") playerMatches[teamA.p2]++;
            if (teamB.p1 !== "BYE") playerMatches[teamB.p1]++;
            if (teamB.p2 !== "BYE") playerMatches[teamB.p2]++;
 ...
 */
export function generateFixed2v2Matches(participants: Participant[]): Match[] {
    const matches: Match[] = [];
    const playerIds = participants.map(p => p.id);
...
export function generateNextDynamic2v2Match(
    pastMatches: Match[],
    ...
    interface PlayerStats {
        id: string;
        amount_times_played: number;
        amount_times_played_in_a_row: number;
        time_since_last_played: number;
        played_with: Set<string>;
    }

    const statsRaw: Record<string, PlayerStats> = {};
    ...
    if (activeParticipants.length < 4) { console.log('less than 4'); return null; }

    const stats = activeParticipants.map(p => statsRaw[p.id]!);

    const getNextPlayer = (candidates: PlayerStats[]): PlayerStats | null => {
