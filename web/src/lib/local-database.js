...
const DATA_DIR = path_1.default.join(__dirname, '../../.local-data');
const DATA_FILE = path_1.default.join(DATA_DIR, 'offline-db.json');
const DEFAULT_STATE = {
    nextIds: {
        users: 1,
        players: 1,
...
function ensureStateFile() {
    ...
    if (!fs_1.default.existsSync(DATA_FILE)) {
        fs_1.default.writeFileSync(DATA_FILE, JSON.stringify(DEFAULT_STATE, null, 2), 'utf-8');
    }
}
function readState() {
    ensureStateFile();
...
function participantFromPlayer(player) {
    return {
...
class LocalDatabase {
    ...
    static async getMatchHistory(userId = null, guestSessionId = 'default-guest') {
        ...
        return state.matches
            ...
            if (match.start_time)
                output.startTime = new Date(match.start_time).getTime();
            if (match.end_time)
                output.endTime = new Date(match.end_time).getTime();
            return output;
        });
    }
    static async saveTournament(mode, matches, userId = null, guestSessionId = 'default-guest') {
        ...
        matches.forEach(match => {
            ...
            team1.participants.forEach(participant => {
                ...
                if (score1 > score2)
                    stats[participant.id].wins++;
                else if (score1 === score2)
                    stats[participant.id].draws++;
                else
                    stats[participant.id].losses++;
            });
    ...
    static async addPlayer(name, userId = null, guestSessionId = 'default-guest') {
        ...
        const duplicateExists = state.players.some(player => {
            if (!matchesScope(player, userId, guestSessionId))
                return false;
